...
| **Assessment application** (`web/`) | Next.js App Router — 4 pages, 6 API routes, 7 checks across 5 assessment areas, deterministic scoring. Authenticates by OAuth 2.0 Client Credentials and reads `Lead` and `Opportunity` by SOQL. 150 unit tests over the checks, the lifecycle controls and the scoring. |
...
`npm test` runs 150 unit tests over the checks, the lifecycle controls and the scoring with no
