...
| Forecast Integrity *(open Opportunities, not a stage)* | `PD-23` — governs only Opportunities promoted into `Best Case` or `Commit`. **Ratified, active in Model v4 and integration runtime validated 2026-09-05; the governed population was empty, so the control reported Not Scored — no applicable records.** |
...
| `PD-23` | **An open Opportunity promoted into `Best Case` or `Commit` must carry the minimum governed evidence NorthstarIQ needs to treat that forecast commitment as quantitatively complete and period-consistent.** The evidence set is **fixed for this MVP**: `Amount` is populated · `CloseDate` falls on or before the end of the Salesforce fiscal quarter containing the assessment date. **`Pipeline` carries no forecast-commitment evidence obligation** under this policy, and `Closed` and `Omitted` are outcome states governed elsewhere (`PD-21`, `PD-17`/`PD-18`). What is **deliberately not decided**: **an override is not itself a defect** — `ForecastCategoryName` is overridable by design and NorthstarIQ never reports a mismatch with the derived `ForecastCategory` as a failure; **`Amount` completeness is evaluated, never correctness**; **`Amount` = 0 is populated** and **no minimum Amount exists**; **no Stage threshold and no Probability threshold exist**. **Past-dated open Opportunities remain `PD-20`'s responsibility** and are never re-scored here — the date failure governed by this policy is a `CloseDate` *after* the period end. **Eventual Won/Lost does not retroactively prove a forecast classification was right or wrong**, and **recognized revenue remains outside NorthstarIQ authority** (`PD-19`). **Adding another required evidence element is a future governed policy decision, not a silent expansion of this one.** | A forecast is only as trustworthy as the records it is assembled from, and *"this deal is expected to land"* and *"this record can be counted"* are different claims. The evidence set is derived from the org's own configuration rather than from a sales methodology. **Every open stage in this org derives `ForecastCategoryName = Pipeline`**, so `Best Case` and `Commit` cannot arise from the configured open-stage defaults — their presence is evidence of deliberate promotion relative to that default, which is what makes the population deterministic without inventing a rule. The org's **only active forecasting type is amount-based** (`OpportunityRevenue`), so NorthstarIQ cannot treat a commitment as quantitatively complete without a governed commercial value; and a commitment dated outside the period it claims contradicts itself on its own two fields. **Nothing here judges seller judgement.** The detector observes CRM state, not actor history: it establishes that a record *is* promoted, never who promoted it or why. | One future **detective** control, *Forecast Commitment Integrity*, over the population `IsClosed = false AND ForecastCategoryName IN ('Best Case','Commit')`. **Detective only, deliberately** — a forecast classification is a human representation, and a preventive rule would refuse a legitimate commercial judgement. The forecast period is resolved from the standard **`Period`** object (the fiscal quarter containing the assessment date), never from `Opportunity.FiscalYear`/`FiscalQuarter`, which discovery found unreliable in the sample data; a period that cannot be resolved uniquely must fail explicitly rather than be invented. Expected to stay bounded to **standard objects with no new custom field, no Flow, no Validation Rule, no Custom Metadata Type and no Record Type**, and to need **no additional access** — `ForecastCategoryName` is not separately permissionable and the integration principal already reads Opportunity. **These are anticipated consequences, not implementation claims.** | ⚠️ **SYNTHETIC — NorthstarIQ portfolio decision**, ratified by the practitioner as owner of the fictional scenario. **No stakeholder approved it.** **High** — detective and read-only, mutating no record and requiring no schema change, so withdrawing it removes a control and changes no Salesforce state. **RATIFIED · REGISTERED AND ACTIVE (Model v4) · SALESFORCE INTEGRATION RUNTIME VALIDATED** (2026-09-05) — one Model v4 assessment executed through the integration principal, resolving the fiscal period from Salesforce's own `Period` records and reading `ForecastCategoryName` successfully. The governed population was **empty at runtime**: `Closed` 19 · `Pipeline` 17 · `Omitted` 1 · **`Best Case` 0 · `Commit` 0**, so the control reported **NOT SCORED — NO APPLICABLE RECORDS**, which is not a pass, not a failure and not 100. ⚠️ **NO LIVE PASS-PATH AND NO LIVE FAIL-PATH EVIDENCE** — neither can exist while the population is empty, and no record was created or promoted to change that. Predicate coverage rests on 37 deterministic tests. Evidence in `testing-strategy.md` §2y. |
