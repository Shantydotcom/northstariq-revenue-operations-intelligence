...
| **Assessment application** (`web/`) | Next.js App Router — 4 pages, 6 API routes, 7 checks across 5 assessment areas, deterministic scoring. Authenticates by OAuth 2.0 Client Credentials and reads `Lead` and `Opportunity` by SOQL. 50 unit tests over the checks and the scoring. |
...
`npm test` runs 50 unit tests over the checks and the scoring with no network and no org. Four
