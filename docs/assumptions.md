...
| `OD-08` | **Forecast Classification Governance** — what business policy, if any, should determine when a human forecast classification is sufficiently supported for NorthstarIQ to verify it independently? | Salesforce owns forecast classification and its native vocabulary: `ForecastCategory` is derived, and `ForecastCategoryName` is overridable **by design**. **NorthstarIQ operates no Forecast Integrity control and judges no classification.** Read-only discovery on 2026-09-04 observed 17 open Opportunities on the native default and **zero forecast-category overrides**, so the Best Case / Commit population that such a rule would govern is currently empty in this org. | Whether any classification carries an evidence obligation, what that evidence is, and whether a detective control is justified at all. Three limits hold regardless of the answer: **`stale-opportunities` is never duplicated under a forecast name**; sales conviction is never converted into an invented deterministic control; and **Forecast Accuracy is never claimed** — it requires period and submission evidence this org does not hold, and cannot be inferred from current-state Opportunity data. A controlled synthetic fixture becomes legitimate only **after** a corresponding requirement is ratified, and is labelled `SYNTHETIC`. |
...
(`OD-01` — never merge; `OD-08` — build no control), or left to the existing safeguard while the
rule is undecided (`OD-06`, `OD-07`). The pattern is the same in each case: **where a business rule
is unagreed, build the capability and leave the rule configurable, rather than inventing the rule.**
